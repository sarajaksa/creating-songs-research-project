...
class FindSong:
    def __init__(
        ...
        self.keys = [
            set([1, 5, 8, 6, 12, 3, 13]),
            set([3, 12, 7, 10, 8, 2, 5, 13]),
            set([6, 10, 1, 11, 3, 5, 8, 13]),
            set([8, 10, 3, 1, 5, 7, 10, 13]),
            set([10, 2, 5, 3, 7, 9, 12, 13]),
            set([5, 9, 12, 10, 2, 4, 7, 13]),
        ]
        ...
        self.all_file_names = []
        if song:
            self.cost_function = self.cost_evaluvation
            self.final_song_lilypond = song
            self.final_song = self.change_representation_to_number(song.split(" "))
            self.duration = len(self.final_song)
        else:
            if type_of_evaluvation == "C Key Cords":
                self.cost_function = self.cost_c_try
            elif type_of_evaluvation == "Article1":
                self.cost_function = self.cost_article1_try
            elif type_of_evaluvation == "Article2-1":
                self.cost_function = self.cost_article2_try1
            elif type_of_evaluvation == "Article2-2":
                self.cost_function = self.cost_article2_try2
            else:
                self.cost_function = self.cost_first_try
            self.duration = duration
            self.final_song = []
        self.songs = [
            self.create_random_sound(self.duration) for i in range(first_generation)
        ]
        self.current_evaluvation = max(
            [self.cost_function(song, self.final_song) for song in self.songs]
        )
        self.iteration = 0
    ...
    def evolutionary_algoritm(self):
        ...
        songs = (
            self.generation(
                songs_crossover + self.songs, self.learning_rate, self.mutation_rate
            )
            + self.songs
            + songs_crossover
        )
        if self.selection == "elite":
            cut_rate = min(
                sorted([self.cost_function(song, self.final_song) for song in songs])[
        ...
        self.songs = [
            song
            for song in songs
            if self.cost_function(song, self.final_song) < cut_rate
        ]
        if not self.songs:
            self.songs = [
                song
        ...
        current_evaluvation = min(
            [self.cost_function(song, self.final_song) for song in self.songs]
        )
        if self.current_evaluvation > current_evaluvation:
            self.current_evaluvation = current_evaluvation
            self.write_music_to_file(
                self.filename,
                self.create_new_song(
                    self.change_representation_to_lilypond(
                        [
                            (note, int(dur))
                            for note, dur in [
                                song
                                for song in self.songs
                                if self.cost_function(song, self.final_song)
                                == self.current_evaluvation
                            ][0]
                        ]
                    )
                ),
        ...
        subprocess.call(
            ["lilypond", "--png", self.filename], stdout=FNULL, stderr=subprocess.STDOUT
        )
    ...
    def cost_first_try(self, song, final_song=None):
        ...
        cost = cost + sum(
            [1 for note1, note2 in zip(song[:-1], song[1:]) if note1[0] == note2[0]]
        )
        # add the cost of notes having too big of a pitch
        cost = cost + sum(
            [
                abs(note1[0] - note2[0]) ** 2
                for note1, note2 in zip(song[:-1], song[1:])
                if not abs(note1[0] - note2[0]) == 1
            ]
        )
    ...
    def cost_c_try(self, song, final_song=None):
        ...
        cost = cost + 10 * sum(
            [1 for note1, note2 in zip(song[:-1], song[1:]) if note1[1] == note2[1]]
        )
        # add the cost of notes having too big of a pitch
        cost = cost + sum(
            [
        ...
        cost = cost + sum(
            [
                abs(note1[1] - note2[1]) if abs(note1[1] - note2[1]) > 1 else 1
                for note1, note2 in zip(song[:-1], song[1:])
            ]
        )
        # add cost of long notes
        cost = cost + 20 * sum(
            [1 / note1[1] if not note1[1] == 0 else 2 for note1 in song]
        )
        # add cost for pauses
    ...
    def cost_article2_try1(self, song, final_song=None):
        ...
        cost = sum(
            [
                1
                / (
                    (self.notes_values[note1[0]] + self.notes_values[note2[0]])
                    / (self.notes_values[note1[0]] * self.notes_values[note2[0]])
                )
                for note1, note2 in zip(song[:-1], song[1:])
                if note1[0] != 13 and note2[0] != 13
            ]
        )
        # add cost of long notes
        cost = cost + sum([1 / note1[1] if not note1[1] == 0 else 2 for note1 in song])
    ...
    def cost_evaluvation(self, song, final_song):
        ...
        """
        result = [
            self.distance_between_points_2D(s, f) for s, f in zip(song, final_song)
        ]
    ...
    def mutation_certain(self, song):
        ...
        """
        random_position, random_type, random_change = (
            random.randint(0, len(song) - 1),
            random.randint(0, 1),
            random.randint(1, 2),
        )
        if (
            random_type == 0
            and random_change == 1
            and song[random_position][random_type] != 1
        ):
            song[random_position] = (
                song[random_position][0],
                song[random_position][1] - 1,
            )
        elif (
            random_type == 1
            and random_change == 1
            ...
            random_type == 0
            and random_change == 2
            and song[random_position][random_type] != 7
        ):
            song[random_position] = (
                song[random_position][0],
                song[random_position][1] + 1,
            ...
            random_type == 1
            and random_change == 2
            and song[random_position][random_type] != 3
        ):
            song[random_position] = (
                song[random_position][0],
                song[random_position][1] + 1,
    ...
    def change_representation_to_number(self, song_lilypond):
        ...
        """
        return [
            (int(self.notes_to_numbers[i[0]]), int(math.log2(int(i[1]))))
            for i in song_lilypond
        ]
    ...
    def change_representation_to_lilypond(self, song_lilypond):
        ...
        return " ".join(
            [
                "".join([n, d])
                for n, d in [
                    (str(self.numbers_to_notes[i[0]]), str(2 ** i[1]))
                    for i in song_lilypond
                ]
            ]
        )

    def create_lilypond_colored_representation(
        self, song_lilypond, final_song_lilypond
    ):
        """
        ...
        for s, f in zip(song_lilypond, final_song_lilypond):
            if s != f:
                final_song.append(
                    "\once \override NoteHead.color = #red \once \override Stem.color = #red"
                )
            final_song.append(s)
    ...
    def create_new_colored_song(self, song, iteration):
        self.create_new_song(
            self.create_lilypond_colored_representation(song, self.final_song_lilypond),
            iteration,
        )
    ...
    def create_new_song(self, song, iteration=None):
        ...
        if iteration:
            music += (
                "\markup {\\fill-line {\\column \\bold  {\line { Iteration: "
                + str(iteration)
                + "}}}}\n\n"
            )
        music += "symbols = {" + song + "}\n"
